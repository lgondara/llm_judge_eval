...
def evaluate_accuracy(model, tokenizer, num_samples=100):
    """
    Evaluate accuracy on MMLU subset (simple QA benchmark).
    Tests if finetuning preserved/improved general capabilities.
    """
    from datasets import load_dataset
    ...
    try:
        dataset = load_dataset(
            "cais/mmlu", 
            "abstract_algebra",  # Small subset
            split="test",
            trust_remote_code=True
        )
    except:
        # Fallback to another simple benchmark
        try:
            dataset = load_dataset("truthful_qa", "multiple_choice", split="validation")
        except:
            print("  Could not load accuracy benchmark, skipping...")
            return None
    ...
    for i in tqdm(range(total), desc="Accuracy"):
        item = dataset[i]
        
        # Format question based on dataset structure
        ...
        # Generate answer
        try:
            response = generate(model, tokenizer, prompt, max_tokens=8)
        except TypeError:
            ...
            inputs = {k: v.to(model.device) for k, v in inputs.items()}
            with torch.no_grad():
                outputs = model.generate(**inputs, max_new_tokens=8, do_sample=False, pad_token_id=tokenizer.pad_token_id)
            generated = outputs[0][inputs["input_ids"].shape[1]:]
            response = tokenizer.decode(generated, skip_special_tokens=True).strip()
...
def evaluate(model_path, test_samples, mmlu_samples=100):
    """Evaluate model on perplexity and accuracy benchmarks."""
    ...
    # Accuracy on MMLU
    accuracy = evaluate_accuracy(model, tokenizer, num_samples=mmlu_samples)
...
def main(args):
    ...
    for strategy_name, scores in all_scores.items():
        ...
        # Evaluate
        metrics = evaluate(model_path, test_samples, mmlu_samples=args.mmlu_samples)
    ...
    print("FINAL RESULTS")
    print("=" * 60)
    print(f"{'Strategy':<15} {'Loss':<10} {'Perplexity':<12} {'MMLU Acc':<10}")
    print("-" * 47)
    for name, metrics in sorted(results.items(), key=lambda x: x[1]["loss"]):
        acc_str = f"{metrics['accuracy']:.1%}" if metrics.get('accuracy') is not None else "N/A"
        print(f"{name:<15} {metrics['loss']:<10.4f} {metrics['perplexity']:<12.2f} {acc_str:<10}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mmlu_samples", type=int, default=100, help="Number of MMLU samples for accuracy eval")
