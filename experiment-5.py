...
ANCHORED_PROMPT = """Compare these two responses to the same instruction. Which is better?
...
Which response is better? Reply with only one of: A++, A+, TIE, B+, B++
(A++ = A much better, A+ = A slightly better, TIE = equal, B+ = B slightly better, B++ = B much better)
...
def score_anchored(model, tokenizer, samples, sample_to_anchor_map):
    """
    Score samples by comparing against anchor response for the SAME prompt.
    
    sample_to_anchor_map: dict mapping sample index to its anchor response
    """
    ...
    for i, sample in enumerate(tqdm(samples)):
        anchor_response = sample_to_anchor_map.get(i)
        if anchor_response is None:
            scores.append(0.5)  # No anchor, default to tie
            continue
...
def build_anchor_map(samples, quality_labels):
    """
    Build mapping from sample index to its anchor (medium-quality response for same prompt).
    
    For anchored comparison:
    - High vs Medium → Target better → B+/B++ → score ~0.75-1.0
    - Medium vs Medium (self) → TIE → score 0.5
    - Low vs Medium → Anchor better → A+/A++ → score ~0.0-0.25
    
    This gives a good spread that should correlate with ground truth quality.
    """
    # Group samples by instruction to find the medium-quality response for each
    instruction_to_medium = {}
    for i, (sample, label) in enumerate(zip(samples, quality_labels)):
        if label == 1:  # Medium quality
            instruction_to_medium[sample["instruction"]] = sample["response"]
    
    # Build anchor map: for each sample, anchor is the medium-quality response for same prompt
    anchor_map = {}
    for i, sample in enumerate(samples):
        instr = sample["instruction"]
        if instr in instruction_to_medium:
            anchor_map[i] = instruction_to_medium[instr]
...
def main(args):
    ...
    # Anchored strategy - compare each response to medium-quality response for SAME prompt
    anchor_map = build_anchor_map(train_samples, train_labels)
    print(f"\nBuilt anchor map: {len(anchor_map)} samples have anchors")
    all_scores["anchored"] = score_anchored(judge_model, judge_tokenizer, train_samples, anchor_map)
